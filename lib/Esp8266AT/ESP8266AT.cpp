/******************************************************************************
ESP8266AT.cpp
Based on SparkFunESP8266WiFi.cpp
...
bool ESP8266Class::begin(EspSerial* serialPort)
{
	_serial = serialPort;
	clearBuffer();
	test();
...
bool ESP8266Class::setBaud(unsigned long baud)
{
	...
	sprintf(parameters, "%d,8,1,0,2", baud);
...
int16_t ESP8266Class::getVersion(char * version)
{
	clearBuffer();
	...
	// (~101 characters)
	// Look for "OK":
	int16_t rsp = readForResponse(RESPONSE_OK2, COMMAND_RESPONSE_TIMEOUT);
...
int16_t ESP8266Class::connect(const char * ssid, const char * pwd)
{
	...
	_serial->print("\r\n");
	while(_serial->tx_fifo.available()) {
		_serial->handle();
	}
...
void ESP8266Class::clearBuffer()
{
	...
	bufferHead = 0;
	_serial->tx_fifo.clear();
	_serial->rx_fifo.clear();
}	
